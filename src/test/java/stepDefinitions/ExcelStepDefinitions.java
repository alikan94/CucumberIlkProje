...
public class ExcelStepDefinitions {
    ...
    public void baskenti_jakarta_olan_ulke_ismini_yazdirir() {
        // for loop ile tum satirlari kontorl etmem gerekiyor
        ...
        for (int i = 0; i <= satirSayisi; i++) {
            if (workbook.getSheet("Sayfa1").getRow(i).getCell(1).toString().equals("Jakarta")){
                System.out.println("başkenti Jakarta olan ulke : " +
    ...
    public void ulke_sayisinin_oldugunu_test_eder(Integer expectedUlkeSayisi) {
        ...
        Assert.assertEquals(actualUlkeSayisi,expectedUlkeSayisi);
    }
    @Then("Fiziki olarak kullanilan satir sayisinin {int} oldugunu test eder")
    public void fiziki_olarak_kullanilan_satir_sayisinin_oldugunu_test_eder(Integer expectedfizikiKullanilanSatir) {

        Integer actualFizKulSatir = workbook.getSheet("Sayfa1").getPhysicalNumberOfRows();
        Assert.assertEquals(actualFizKulSatir,expectedfizikiKullanilanSatir);
